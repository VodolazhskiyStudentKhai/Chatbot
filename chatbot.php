...
if (strpos($message, '/start') === 0) {
    ...
    if ($param === null) {
        getMainKeyboard($chatId, MSG_WELCOME, $role);
    } else {
...
function getMainKeyboard($chatId, $text, $isAdmin) {
    if ($isAdmin == 1) {
        $keyboard = [
            'keyboard' => [
                [BUTTON1, BUTTON4],
...
function sendTests($chatId, $page = 0, $stat = 0) {
	...
	sendTelegramRequest($chatId, MSG_TESTAV, ['reply_markup' => $keyboard]);
}
...
function genEventQR($chatId, $eventId) {
    ...
    genQR($chatId, "Запрошення №{$eventId}", $link);
}
